...
void CTFHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		{
			// Draw killer's name
			Color clr = TFGameRules()->IsDMGamemode() ? tf_PR->GetPlayerColor(msg.Killer.iPlayerID) : GetTeamColor(msg.Killer.iTeam, msg.bLocalPlayerInvolved);
		...
		if ( assister[0] )
		{
			...
			Color clr = TFGameRules()->IsDMGamemode() ? tf_PR->GetPlayerColor(msg.Assister.iPlayerID) : GetTeamColor(msg.Assister.iTeam, msg.bLocalPlayerInvolved);
		...
		// Draw victims name
		Color clr = TFGameRules()->IsDMGamemode() ? tf_PR->GetPlayerColor(msg.Victim.iPlayerID) : GetTeamColor(msg.Victim.iTeam, msg.bLocalPlayerInvolved);

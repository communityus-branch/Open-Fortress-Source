...
void CGrabController::DetachEntity( bool bClearVelocity )
{
	...
	if ( pEntity )
	{
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			{
#ifndef CLIENT_DLL
				ClampPhysicsVelocity( pPhys, 320 * 1.5f, 2.0f * 360.0f );
#endif
			}
...
LINK_ENTITY_TO_CLASS( ofd_weapon_physgauntlet, CWeaponPhysCannon );
PRECACHE_WEAPON_REGISTER( ofd_weapon_physgauntlet );
...
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	...
	CTFPlayer *pOwner = (CTFPlayer *)ToBasePlayer( GetOwner() );
